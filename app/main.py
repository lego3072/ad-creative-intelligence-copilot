...
DB_PATH = DATA_DIR / "ad_creative_intelligence_copilot.db"

DATA_DIR.mkdir(parents=True, exist_ok=True)

APP_NAME = "Ad Creative Intelligence Copilot"
APP_SLUG = "creativeintelapi"
DEFAULT_BASE_URL = "https://ad-creative-intelligence-copilot.up.railway.app"
...
FOLLOWUP_FROM_EMAIL = os.getenv("FOLLOWUP_FROM_EMAIL", "Ad Creative Intelligence Copilot <joseph@dataweaveai.com>").strip()
...
STARTER_PRICE_LABEL = os.getenv("STARTER_PRICE_LABEL", "$119/mo").strip()
DFY_PRICE_LABEL = os.getenv("DFY_PRICE_LABEL", "$3k one-time").strip()
...
STARTER_MONTHLY_COST_CAP_USD = float(os.getenv("STARTER_MONTHLY_COST_CAP_USD", "23.8"))
...
def request_access_key(payload: AccessKeyRequest, request: Request) -> dict[str, Any]:
    ...
    if row and row["status"] in ACTIVE_ACCOUNT_STATUSES:
        ...
        if new_key:
            ...
            send_resend_email(
                subject=f"{APP_NAME} key rotated",
                html=f"<p>API key rotated for {email}</p>",
            )
...
def intent_checkout(intent_id: str, payload: IntentCheckoutRequest, request: Request) -> dict[str, Any]:
    require_paid_access(request)
    checkout_url = checkout_link_for_plan(payload.plan)
    with get_conn() as conn:
